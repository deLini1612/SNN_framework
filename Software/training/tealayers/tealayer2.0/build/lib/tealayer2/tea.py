...
def custom_clip(x):
    ...
    def grad(dy, variables=None):
        grad_x = dy * tf.cast((x > 0.5) | (x <= 0.5), tf.float32)
...
@tf.custom_gradient
def custom_round(x):
    """Feedforward: round(x), Backprop: identity gradient (1)"""
    y = tf.round(x)
...
class Tea(layers.Layer):
    ...
    def __init__(self,
                 ...
                 round_input=None,
                 round_connections=None,
                 clip_connections=None,
                 round_bias=None,
                 constrain_after_train=True,
                 activation_factor=1.0, # custom_sigmoid
                 **kwargs):
        ...
        if connection_initializer:
            ...
            self.connection_initializer=initializers.TruncatedNormal(mean=0.41, stddev=0.075)
        ...
        if bias_initializer:
            ...
            self.bias_initializer=initializers.RandomUniform(minval=-2.0, maxval=2.0, seed=None)
    ...
    def call(self, inputs, training=None):
        ...
        with tf.compat.v1.get_default_graph().gradient_override_map({"Round":"CustomRound"}):
            ...
            # else:
            #     connections = 1 / (1 + tf.exp(-1.0 * connections))
            ...
            potential = tf.keras.backend.dot(inputs, weighted_connections)
            ...
            if self.use_bias:
                ...
                potential = tf.keras.backend.bias_add(potential, biases, data_format='channels_last')

            # Apply activation / Spike(s)
            # output = self.activation(potential - self.threshold)
            output = tf.cast(tf.greater_equal(potential, self.threshold), tf.float32) if not training else self.activation(potential) 
