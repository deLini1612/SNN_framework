...
def create_cores(
    model, num_layers, core_coordinates,
    weights=[1, -1, 1, -1],
    ...
    cores = [] #di chuyển lên để có thể dùng cả 2 trường hợp là core_coordinates = None hoặc not None
    ...
    for layer_coordinates, layer_shapes, connections_layer, bias_layer \
            in zip(core_coordinates, core_shapes, connections, biases):
        layer_cores = []
        for single_core_coordinates, core_shape, connections_core, bias_core \
                in zip(layer_coordinates, layer_shapes, connections_layer, bias_layer):
    ...
    layer_num = 0
    for layer_cores, layer_shapes, layer_size, connections_layer, bias_layer \
            in zip(cores, core_shapes, layer_sizes, connections, biases):
        core_offset = 0
        axon_offset = 0
        for core, core_shape, connections_core, bias_core \
                in zip(layer_cores, layer_shapes, connections_layer, bias_layer):
            neurons = []
            for connections_neuron, bias_neuron in zip(connections_core.T, bias_core):
                if layer_num != len(cores) - 1:
                    coords = (cores[layer_num + 1][core_offset].coordinates[0] - core.coordinates[0],
                              cores[layer_num + 1][core_offset].coordinates[1] - core.coordinates[1])
                else:
                    coords = (1, 0) # để dx = 1 và dy = 0 để tạo thành core ouput bus nằm ở vị trí 2,1
                ...
                if layer_num < len(core_shapes) - 1:
                    if axon_offset == core_shapes[layer_num + 1][core_offset][0]:
                        core_offset += 1
                        axon_offset = 0
...
# def create_packets(packets, core_coordinates):
#     # Ensure core_coordinates are in 2D format
#     if core_coordinates is not None and len(core_coordinates[0]) == 4: # kiểm tra xem tầng 1 có đủ 4 core không
#         core_coordinates = [coord for coord in core_coordinates]
...
#             for packet_num, packet in enumerate(tick):
#                 if np.round(packet) == 1: 
#                     if (core_num, 0) in core_coordinates: # nếu core ở tàng 1 có dy = 0 để xác dịnh 3 core dầu tiên của tầng 1
#                         packets_tick.append(
#                             Packet(destination_core=(core_num, 0),
#                                 destination_axon=packet_num,
#                                 destination_tick=0)
#                         ) 
#                     else:
#                         packets_tick.append( # tức là dy = 1 thì để xác định core cuối cùng của tầng 1, từ đó gán dx và dy trong neuron của core này cho phù hợp 
#                             Packet(destination_core=(0, 1),
#                                 destination_axon=packet_num,
#                                 destination_tick=0)
#                         )
