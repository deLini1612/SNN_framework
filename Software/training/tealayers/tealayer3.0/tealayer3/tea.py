...
class Tea(layers.Layer):
    ...
    def call(self, inputs):
        ...
        with tf.compat.v1.get_default_graph().gradient_override_map({"Round":"CustomRound"}):
            ...
            self.potential = tf.keras.backend.dot(inputs, weighted_connections)
            ...
            if self.use_bias:
                ...
                self.potential = tf.keras.backend.bias_add(self.potential, biases, data_format='channels_last')

            # Apply activation / Spike(s)
            output = tf.keras.backend.in_train_phase(self.activation(self.potential), 
                                                     tf.dtypes.cast(tf.math.greater_equal(self.potential, 0.0), tf.float32))
